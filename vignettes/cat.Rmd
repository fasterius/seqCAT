...
chromosome 12 (containing all variants up to position `25400000`) for three
different colorectal cancer cell lines: *HCT116*, *HKE3* and *RKO*.
...
create_profile(file, "HCT116", "hct116_profile.txt")
create_profile(file, "RKO", "rko_profile.txt", filter_depth = 15)
```
...
create_profile(file, "RKO", "RKO_profile.txt", python = TRUE)
```
...
overlaps <- overlap_profiles(hct116, rko)
```
...
create_profile(file, "HKE3", "hke3_profile.txt")
hke3 <- read_profile("hke3_profile.txt", "HKE3")

# Overlap HCT116 and HKE3
overlaps_hct116_hke3 <- overlap_profiles(hct116, hke3)
overlaps_hct116_hke3 <- compare_overlaps(overlaps_hct116_hke3)

# Add HCT116/HKE3 similarities to HCT116/RKO similarities
similarities <- calculate_similarity(overlaps_hct116_hke3,
                                     similarity, a = 1, b = 10)
similarities
```
