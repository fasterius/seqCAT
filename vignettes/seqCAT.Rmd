...
version of seqCAT on [GitHub][github], which can be installed like so:
...
parameters each SNV profile is saved on the harddrive as a normal `.txt` file.
...
profiles is also implemented in Python ([section 2.2][Create profiles faster
with Python]), which is faster than the standard implementation in R ([section
...
create_profile(vcf, "RKO", "rko.profile.txt", filter_depth = 15)
```
...
`rko.profile.txt` in the current directory, respectively. The profile of the
...
Annotated SNV profiles can also be created with [Python][python-home], another
...
which will create SNV profiles for each VCF file in a given input directory.
You can use it for all the VCFs in a directory, or a subset specified by a
string, like so:
...
```{r Remove unused profiles from multiple profile creation, echo = FALSE, results = "hide"}
...
has two functions to help with this: `list_variants` and `plot_variant_list`.
...
## Reading multiple profiles

The `read_profiles` function is a convenience-wrapper for `read_profile`, which
will automatically read all the profiles present in a given directory and
return them as a list, which is then easily parsed to the `compare_many`
function.

```{r Read multiple profiles}
...
file.remove("rko.profile.txt")
file.remove("hke3.profile.txt")
```
