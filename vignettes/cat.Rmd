...
[here][7]. As redistributing this file is not allowed by the COSMIC team, this
package includes an extremely minimal subset of the original file, only useful
for examples in this vignette and unit testing. *Do not* use this file for your
own analyses, as your results will neither be complete nor correct!

[6]: http://cancer.sanger.ac.uk/cosmic
[7]: http://cancer.sanger.ac.uk/cell_lines/download
...
many-to-one or many-to-many cases - the `CellAuthentication` package provides a
...
If you are interested in only a one-to-many comparison (for cases when you have
a "true" baseline profile to compare against), you can do this by also
specifying the `one = <profile>` parameter in the function call. You may also
specify the `a` and `b` similarity score parameters, as above. 
