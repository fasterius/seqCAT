...
```{r Compare profiles}
comparison_hct116_rko <- compare_profiles(hct116, rko)
head(comparison_hct116_rko)
```
...
similarity <- calculate_similarity(comparison_hct116_rko)
similarity
```
...
impacts <- plot_impacts(comparison_hct116_rko)
impacts
```

We can see that most of the SNVs are present in the MODIFIER impact category,
and that there is not a single mismatched HIGH impact SNV.
